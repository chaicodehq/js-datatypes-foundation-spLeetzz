...
export function findCheapestAndCostliest(...fares) {
  let numbers = [];
  for (const arg of fares) {
    if (typeof arg === "number" && !isNaN(arg) && Number.isInteger(arg)) {
      numbers.push(arg);
    }
  }
  console.log(numbers);
  ...
  return { cheapest: Math.min(...numbers), costliest: Math.max(...numbers) };
}
